...
public class MaxHeap : MonoBehaviour
{
	...
	void Update()
	{
		TreeUtils.CheckConnectionsMax(connections);
	}
	...
	private IEnumerator HeapifyDown()
	{
		...
		while (HasLeftChild(index))
		{
			...
			// If current node's value is greater than child node's value then done
			if (items[index].Value > items[largerChildIndex].Value)
			{
				// Animation: Change colors
				items[index].ChangeColor(TreeUtils.GREEN);
				items[largerChildIndex].ChangeColor(TreeUtils.GREEN);
				arrayBox.Items[index].ChangeColor(TreeUtils.GREEN);
				arrayBox.Items[largerChildIndex].ChangeColor(TreeUtils.GREEN);
				yield return new WaitForSeconds(animationSpeed);

				// Animation: Change colors
				items[index].ChangeColor(TreeUtils.TRANSPARENT);
				items[largerChildIndex].ChangeColor(TreeUtils.TRANSPARENT);
				arrayBox.Items[index].ChangeColor(TreeUtils.TRANSPARENT);
				arrayBox.Items[largerChildIndex].ChangeColor(TreeUtils.TRANSPARENT);
				break;
			}
			else
			{
	...
	private IEnumerator HeapSort(int size)
	{
		...
		{
			Coroutine a = StartCoroutine(HeapifyMax(i, size));
			yield return a;
		}
		working = false;
	}

	private IEnumerator HeapifyMax(int index, int size)
	{
		int smallest = index;
		int l = GetLeftChildIndex(index);
		int r = GetRightChildIndex(index);
		...
		if (l < size && items[smallest].Value < items[l].Value)
		{
			smallest = l;
			items[l].ChangeColor(TreeUtils.YELLOW);
			arrayBox.Items[l].ChangeColor(TreeUtils.YELLOW);
		...
		if (r < size && items[smallest].Value < items[r].Value)
		{
			smallest = r;
			items[l].ChangeColor(TreeUtils.TRANSPARENT);
			items[r].ChangeColor(TreeUtils.YELLOW);
		...
		if (smallest != index)
		{
			...
			Coroutine a = StartCoroutine(Swap(index, smallest));
			yield return a;
			Coroutine b = StartCoroutine(HeapifyMax(smallest, size));
			yield return b;
		}
